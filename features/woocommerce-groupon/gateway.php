<?php
namespace Essentials\Features\WooCommerce\Gateways\Groupon;
...
final class Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->id                 = 'groupon';
		$this->method_title       = esc_html_x( 'Groupon', 'brand', 'essentials' );
		...
		<fieldset>
			<div class="form-row form-row-wide">
				...
				<input type="text"
					   class="input-text"
					   id="groupon_vouchers"
					   name="groupon_vouchers"
					   placeholder="123456 <?php echo esc_attr_x( 'or', 'function word', 'essentials' ); ?> 1234567, 12345678"
				/>
			</div>
			<div class="clear"></div>
		</fieldset>
	...
	public function process_payment( $order_id ) {
		...
		$groupon_vouchers           = $this->get_post( 'groupon_vouchers' );
		$groupon_vouchers_no_spaces = preg_replace( '/\s+/m', '', $groupon_vouchers );

		if ( isset( $groupon_vouchers ) ) {
			update_post_meta( $order_id, '_groupon_vouchers', esc_attr( $groupon_vouchers_no_spaces ) );
		}
	...
	public function display_order_information( $order ) {
		if ( $order->get_payment_method() === 'groupon' ) {
			$output           = '';
			$title            = esc_html_x( 'Groupon', 'brand', 'essentials' );
			$groupon_vouchers = get_post_meta( $order->get_order_number(), '_groupon_vouchers', true );

			if ( $groupon_vouchers != '' ) {
				$groupon_vouchers_display = preg_replace( '/,(\s*)/m', ', ', $groupon_vouchers );

				$output .= '<p class="form-field form-field-wide">';
				$output .= '<label>' . $title . ': </label>';
				$output .= '<span style="color: #333; font-size: 20px;">' . $groupon_vouchers_display . '</span>';
				$output .= '</p>';
			}
	...
	public function validate_fields() {
		...
		if ( ! preg_match( '/^[0-9]{6,8}(, *[0-9]{6,8})*$/', $groupon_vouchers ) ) {
			wc_add_notice(
				sprintf(
					esc_html__(
